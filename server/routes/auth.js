...
router.post(
  ...
  loginLimiter,
  [
    body('email').isEmail().withMessage('Email invalide.').normalizeEmail().trim(),
    body('password').isLength({ min: 6 }).withMessage('Mot de passe trop court.').trim(),
  ],
  async (req, res) => {
...
router.post(
  ...
  [
    ...
    body('newPassword').optional({ checkFalsy: true })
      ...
      .matches(/[0-9]/).withMessage('Doit contenir au moins un chiffre.').trim(),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ error: errors.array()[0].msg });
    ...
    if (email !== admin.email) {
      const existing = await get('SELECT id FROM admin WHERE email = ? AND id != ?', [email, req.admin.id]);
      if (existing) return res.status(400).json({ error: 'Cette adresse email est déjà utilisée.' });
    }

    if (newPassword) {
      const newHash = await bcrypt.hash(newPassword, 12);
      await run('UPDATE admin SET password = ? WHERE id = ?', [newHash, req.admin.id]);
    }

    await run('UPDATE admin SET email = ? WHERE id = ?', [email, req.admin.id]);

    const token = jwt.sign({ id: admin.id, email }, JWT_SECRET, { expiresIn: '8h' });
    res.cookie('token', token, {
      httpOnly: true,
      sameSite: 'strict',
      maxAge: 8 * 60 * 60 * 1000,
